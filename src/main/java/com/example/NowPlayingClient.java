...
public class NowPlayingClient implements ClientModInitializer, ModMenuApi {
    ...
    public void onInitializeClient() {
                ...
                if (csharpProcess != null && csharpProcess.isAlive()) {
                    try {
                        ...
                        if (status == 200) {
                            ...
                            isMediaActive = isActiveStatus
                                    && !newMediaTitle.isEmpty()
                                    && !newMediaTitle.equals("(none)")
                                    && !newMediaTitle.equals("(unknown)");
        ...
        HudRenderCallback.EVENT.register((drawContext, tickDelta) -> {
            ...
            int panelX = (config.sidePosition == NowPlayingConfig.Side.LEFT)
                    ? 0
    ...
    private boolean isDisplayableMedia() {
        return mediaTitleText != null
                && !mediaTitleText.isEmpty()
                && !mediaTitleText.equals("Loading Now Playing...")
                && targetProgress > 0
                && targetEndSec > 0.0;
    }
